...
def prepareDataset(validDataFileName, invalidDataFileName, dataSplit, invalidCount=1000, useValidCount=False) -> tuple:
    ...
    validDataframe = np.concatenate((validDataframe.values, validDataframe.values, validDataframe.values, validDataframe.values))
    ...
    invalidDataframe = invalidDataframe.values
...
def sliceDataset(data) -> tuple:
    ...
    # one-hot encode data
    output_dim = 2
    yTrain = tf.keras.utils.to_categorical(yTrain, output_dim)
    yTest = tf.keras.utils.to_categorical(yTest, output_dim)
...
def logisticRegressionModel(data) -> None:
    ...
    model.add(tf.keras.layers.Dense(output_dim, input_dim=input_dim, activation='softmax'))
...
def shallowNeuralNetworkModel(data) -> None:
    ...
    # build model
    model = tf.keras.models.Sequential()
    model.add(tf.keras.layers.Dense(layer_one_size, input_dim=input_dim, activation='tanh'))
    model.add(tf.keras.layers.Dense(output_dim, activation='softmax'))

    # compile model
    model.summary()
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy', tf.keras.metrics.TruePositives()]) 
    model.fit(xTrain, yTrain, batch_size=batchSize, epochs=numEpochs, verbose=True, validation_data=(xTest, yTest))
    return model
...
def deepNeuralNetworkModel(data, verbose=False) -> None:
    ...
    # build model
    model = tf.keras.models.Sequential()
    model.add(tf.keras.layers.Dense(layer_zero_size, input_dim=input_dim, activation='tanh'))
    model.add(tf.keras.layers.Dropout(0.01))
    model.add(tf.keras.layers.Dense(layer_one_size, activation='tanh'))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Dense(layer_two_size, activation='tanh'))
    model.add(tf.keras.layers.Dropout(0.01))
    model.add(tf.keras.layers.Dense(layer_three_size, activation='tanh'))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Dense(layer_four_size, activation='tanh'))
    model.add(tf.keras.layers.Dropout(0.01))
    model.add(tf.keras.layers.Dense(layer_five_size, activation='tanh'))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Dense(output_dim, activation='softmax'))

    # add callback for early-stopping
    stop = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=6, verbose=verbose, mode='min')

    # compile model
    model.summary()
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.001, beta_1=0.9, beta_2=0.999, amsgrad=False)
    model.compile(optimizer=optimizer, loss='poisson', metrics=['accuracy', tf.keras.metrics.TruePositives()]) 
